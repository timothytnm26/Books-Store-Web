...
namespace Project8.Areas.Admin.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        [HttpPost]
        public ActionResult ChangeInfo(FormCollection model)
        {
            var u = Session["LoginAdmin"];
            var r = db.Admins.FirstOrDefault(x => x.TaiKhoan == u);

            string mk = model["MatKhau"];
            string newPass = model["NewPass"];
            string repass = model["RePass"];
            string hoten = model["HoTen"];
            ...
            r.HoTen = hoten;
            var rs = new UserProcess().UpdateAdmin(r);
            ViewBag.Success = "Thay đổi thông tin thành công";
            return View("AdminInfo", r);
        }
    }
}